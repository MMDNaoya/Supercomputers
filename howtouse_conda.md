...
## TL;DR
- **問題:** GPUノード（aarch64）とCPUノード（x86_64）でアーキテクチャが異なるため、通常の `conda init` は不整合を起こす．
...
| :--- | :--- | :--- | :--- |
| **GPUノード** | `aarch64` | ARM系 | GPU計算，AI学習 |
| **CPUノード** | `x86_64` | AMD系 | CPU並列計算 |
...
Conda で作成した仮想環境（env）自体には互換性がない．
    ...
    解決策: 環境名にアーキテクチャを明記して管理することを推奨する．
        ...
        例: myenv_gpu (GPUノード用)，myenv_cpu (CPUノード用)
